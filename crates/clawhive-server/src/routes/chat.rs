    ...
    loop {
        ...
        while let Ok(msg) = rx_reply.try_recv() {
            if let BusMessage::ReplyReady { outbound } = msg {
                ...
                if !is_active_trace_id(&active_trace_ids, trace_id).await {
                    tracing::trace!(trace_id = %trace_id, "dropping reply_ready for inactive trace_id");
                    continue;
                }
...
async fn list_conversations(
    ...
    for (agent_id, sessions_dir) in workspace_sessions_dirs(&state.root) {
        ...
        for entry in entries.flatten() {
            ...
            if !is_web_console_user_session(&session_key, &token_prefix) {
                continue;
            }
...
async fn delete_conversation(
    ...
    let Some(path) = find_conversation_session_file(&state.root, &id, &token_prefix) else {
...
fn workspace_sessions_dirs(root: &Path) -> Vec<(String, PathBuf)> {
...
fn is_web_console_user_session(session_key: &str, user_token_prefix: &str) -> bool {
    session_key.contains("web_console")
        && session_key.contains(&format!("user:web_{user_token_prefix}"))
}

fn conversation_id_from_session_key(session_key: &str) -> Option<String> {
...
fn summarize_session_content(content: &str) -> (usize, Option<String>, Option<String>) {
...
fn find_conversation_session_file(
    root: &Path,
    conversation_id: &str,
    user_prefix: &str,
) -> Option<PathBuf> {
...
mod tests {
    ...
    fn conversation_id_from_session_key_extracts_id() {
        assert_eq!(
            super::conversation_id_from_session_key(
    ...
    fn conversation_id_from_session_key_returns_none_for_non_chat() {
        assert_eq!(
            super::conversation_id_from_session_key("telegram:bot123:group:456:user:user789"),
            None
        );
    }
    ...
    fn is_web_console_user_session_matches_correctly() {
        ...
        assert!(super::is_web_console_user_session(key, "abcd1234"));
        assert!(!super::is_web_console_user_session(key, "other123"));
    }

    #[test]
    fn token_prefix_returns_first_8_chars() {
        assert_eq!(super::token_prefix("abcdefghijklmnop"), "abcdefgh");
        assert_eq!(super::token_prefix("short"), "short");
    }
    ...
    fn summarize_session_content_counts_messages() {
        ...
        let (count, last_at, preview) = super::summarize_session_content(content);
        assert_eq!(count, 2);
    ...
    fn parse_conversation_messages_handles_tool_calls() {
        ...
        let messages = super::parse_conversation_messages(content);
