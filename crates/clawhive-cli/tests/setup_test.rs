...
fn unique_test_root() -> std::path::PathBuf {
    std::env::temp_dir().join(format!("clawhive-init-test-{}", Uuid::new_v4()))
}
...
fn test_standard_config_generation() -> Result<()> {
    ...
    fs::create_dir_all(root.join("prompts/clawhive-main"))?;
...
"#,
    ...
    fs::write(
        root.join("config/agents.d/clawhive-main.yaml"),
        r#"agent_id: clawhive-main
enabled: true
identity:
  name: "Clawhive"
  emoji: "🦀"
...
"#,
    ...
    fs::write(
        ...
        r#"default_agent_id: clawhive-main
...
"#,
    ...
    fs::write(
        root.join("prompts/clawhive-main/system.md"),
