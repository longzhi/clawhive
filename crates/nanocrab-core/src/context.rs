...
pub fn estimate_message_tokens(msg: &LlmMessage) -> usize {
    ...
    for block in &msg.content {
        match block {
            nanocrab_provider::ContentBlock::Text { text } => {
                total += estimate_tokens(text);
            }
            nanocrab_provider::ContentBlock::ToolUse { input, .. } => {
...
pub fn prune_tool_results(
    ...
    for msg in messages.iter_mut().take(len - keep_last_n) {
        for block in msg.content.iter_mut() {
            if let nanocrab_provider::ContentBlock::ToolResult { content, .. } = block {
...
pub async fn compact_messages(
    ...
    for msg in to_compact {
        let role = &msg.role;
        for block in &msg.content {
            if let nanocrab_provider::ContentBlock::Text { text } = block {
...
mod tests {
    ...
    fn test_prune_tool_results() {
        let mut messages = vec![
            LlmMessage {
                role: "user".into(),
                content: vec![nanocrab_provider::ContentBlock::ToolResult {
                    tool_use_id: "1".into(),
        ...
        if let nanocrab_provider::ContentBlock::ToolResult { content, .. } = &messages[0].content[0]
        {
